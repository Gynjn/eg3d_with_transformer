...
@click.option('--cmax',         help='Max. feature maps', metavar='INT',                        type=click.IntRange(min=1), default=128, show_default=True)     # 수정 전 : default=512
@click.option('--glr',          help='G learning rate  [default: varies]', metavar='FLOAT',     type=click.FloatRange(min=0))
@click.option('--dlr',          help='D learning rate', metavar='FLOAT',                        type=click.FloatRange(min=0), default=0.002, show_default=True)
@click.option('--map-depth',    help='Mapping network depth  [default: varies]', metavar='INT', type=click.IntRange(min=1), default=8, show_default=True)   # 수정 전 : default=2
@click.option('--mbstd-group',  help='Minibatch std group size', metavar='INT',                 type=click.IntRange(min=1), default=4, show_default=True)

# Misc settings.
@click.option('--desc',         help='String to include in result dir name', metavar='STR',     type=str)
@click.option('--metrics',      help='Quality metrics', metavar='[NAME|A,B,C|none]',            type=parse_comma_separated_list, default='fid50k_full', show_default=True)
@click.option('--kimg',         help='Total training duration', metavar='KIMG',                 type=click.IntRange(min=1), default=5000, show_default=True)      # 수정 전 : default = 25000
...
@click.option('--neural_rendering_resolution_final', help='Final resolution to render at, if blending', metavar='INT',  type=click.IntRange(min=1), required=False, default=64)   # 수정 전 : None
...
@click.option('--num_layers', help='Number of transformer encoder_layer', type=list, default = [1,2,2,2], show_default=True)      # 수정 전 : [1,3,3,3,3,3]
...
def main(**kwargs):
    ...
    c.G_kwargs = dnnlib.EasyDict(class_name= None, z_dim=128, w_dim=128, mapping_kwargs=dnnlib.EasyDict(), synthesis_kwargs=dnnlib.EasyDict())      # 수정 전 : z_dim=512, w_dim=512
    ...
    if c.training_set_kwargs.resolution == 512:
        ...
        sr_module = 'training.superresolution.SuperresolutionHybridX'
    else:
