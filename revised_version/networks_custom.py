...
class EncoderBlock(nn.Module):
    def __init__(self, h_dim, w_dim, out_dim, depth, minimum_head, img_resolution, resolution, img_channels, is_first, is_last, architecture='skip', linformer=False, conv_clamp=None, use_fp16=False, fp16_channels_last=False, resample_filter =[1,3,3,1], scale_ratio=2, **layer_kwargs):
        ...
        self.enc = Encoderlayer(h_dim=self.h_dim, w_dim=self.w_dim, out_dim=self.out_dim, seq_length=self.seq_length, depth=self.depth, minimum_head=self.minimum_head, conv_clamp=self.conv_clamp, proj_weight=self.proj_weight,**layer_kwargs)    # 수정 전 : **layer_kwargs 없었음
    ...
    def forward(self, x, img, ws, force_fp32=True, fused_modconv=None, update_emas=False, **layer_kwargs):
...
class SynthesisNetwork(nn.Module):
    def __init__(self, w_dim, img_resolution, img_channels, depth, minimum_head, num_layers, G_dict, conv_clamp, channel_base = 8192, channel_max = 128, num_fp16_res = 0, linformer=False):    # 수정 전 : 256
    ...
    def forward(self, ws=None, **block_kwargs):
        ...
        for i, (res, cur_ws) in enumerate(zip(self.block_resolutions, block_ws)):
            ...
            for j in range(num_block_res):
                block = getattr(self, f'b{res}_{j}')
                x, img = block(x, img, cur_ws[j], **block_kwargs)
